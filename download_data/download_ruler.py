...
def to_serializable(obj):
    """Convert numpy / non-standard types to JSON-serializable types."""
...
def split_input(text: str, dataset: str):
    """
    Split input by dataset type into (example, instruction, context, question).
    """
    ...
    if dataset == "cwe":
        ...
        # Find all instruction positions
        instr_positions = []
        start = 0
        while True:
        ...
        if instr_positions:
            # Use the last occurrence as the actual instruction
            instr_start = instr_positions[-1]
            instr_end = instr_start + len(cwe_instr)
            instruction = cwe_instr

            # Treat previous content as example (may include full QA)
            if instr_start > 0:
                example = text[:instr_start].strip()

            # Find the first \"Question:\" after instruction
            q_pos = text.find("Question:", instr_end)
            if q_pos != -1:
                context = text[instr_end:q_pos].strip()
                question = text[q_pos:].strip()
            else:
                # No Question: treat the rest as context
                context = text[instr_end:].strip()
        else:
            # No fixed instruction found; simple split into context + question
    ...
    if dataset in ("qa_1", "qa_2"):
        ...
        q_pos = text.rfind("Question:")
        if q_pos != -1:
            # question starts from the last QA instruction or \"Question:\"
            context = text[instr_end:q_pos].strip()
            question = text[q_pos:].strip()
        else:
            context = text[instr_end:].strip()
...
def process_dataset(dataset, dataset_name: str, output_path: str):
    """Process one dataset subset and save as jsonl."""
    text_col = None
    ...
    with open(output_path, "w", encoding="utf-8") as f_out:
        for row in dataset:
            ...
            # Keep all non-text columns (e.g., index / outputs)
            for k in cols:
                if k == text_col:
                    continue
                v = row[k]
