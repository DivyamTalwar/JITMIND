...
# -*- coding: utf-8 -*-
"""
JITMind + NarrativeQA evaluation script.

Based on test_gam_hotpotqa.py, adapted to NarrativeQA format.
NarrativeQA format:
- document: dict with text/id/summary
- question: dict with text
- answers: List[dict] with text fields
"""
...
def load_narrativeqa(data_dir: str, split: str = "test") -> List[Dict[str, Any]]:
    ...
    Args:
        data_dir: dataset directory containing parquet files
        split: dataset split ("train", "validation", "test")
    """
    from datasets import load_dataset
    
    # Load dataset
    print(f"Loading dataset: {data_dir}, {split}")
    dataset = load_dataset("parquet", data_dir=data_dir, split=split)
    
    print(f"Loaded successfully. Dataset size: {len(dataset)}")
    ...
    for idx, item in enumerate(dataset):
        ...
        # Extract answers list (from answers[*].text)
        ...
        data_all.append({
            ...
            "answers": answers,
            "_id": f"narrativeqa-{document_id}-{idx}"  # unique ID
        })
...
def build_context_chunks_for_sample(
    ...
    """
    Split document_text into multiple chunks by token count.
    Uses smarter splitting with boundary preference.
    
    Args:
        sample: sample data containing 'document_text'
    ...
    # Prefer precise token split with embedding model if provided
    ...
    # Use tiktoken for precise token splitting
    try:
        import tiktoken
...
def _split_with_embedding_model(text: str, max_tokens: int, model_path: str) -> List[str]:
    ...
    try:
        ...
        # Use tokenizer from the specified model
...
def _smart_split_by_tokens(text: str, tokens: List[int], max_tokens: int, tokenizer) -> List[str]:
    ...
    # If text is within max_tokens, return directly
    if len(tokens) <= max_tokens:
        return [f"[Session 1]\n{text}"]
...
def process_sample(
    ...
    3. Answer the question based on the research result
    """
...
def main():
    ...
    parser = argparse.ArgumentParser(description="JITMind + NarrativeQA evaluation")
    parser.add_argument("--data-dir", type=str, default="/path/to/narrativeqa/data", 
                        help="NarrativeQA dataset directory")
    ...
    print("=" * 60)
    print("JITMind + NarrativeQA evaluation")
    print("=" * 60)
